...
@section('content')
    ...
    <main>
        ...
        @foreach($categories as $category)
            @if(isset($postsByCategory[$category->name]) && count($postsByCategory[$category->name]) > 0)
                                        ...
                                        <div class="relative h-62 w-full mb-3">
                                            <div class="absolute flex flex-col top-0 right-0 p-3">
                                                ...
                                                @if ($favoris && $favoris->contains($post->id))
                                                    {{-- Display form to remove from favorites --}}
                                                    <form action="{{ route('removeToFavoris') }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <input type="hidden" value="{{$post->id}}" name="postId">
                                                        <button type="submit"
                                                                class="transition ease-in duration-300 bg-gray-800 text-red-500 shadow hover:shadow-md hover:text-gray-500 rounded-full w-8 h-8 text-center p-1">
                                                    ...
                                                    {{-- Check if post is not in favorites --}}
                                                @elseif ($favoris && !$favoris->contains($post->id))
                                                    {{-- Display form to add to favorites --}}
